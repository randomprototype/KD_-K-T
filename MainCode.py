...
def main():
    ...
    if choice == menu[0]:
        ...
        ci = st.number_input(
    "Insert cost of inspection ($C_{I}$)", 
    min_value=0.0, value=0.05, 
    help="This parameter represents the cost of conducting an inspection."
)

cr = st.number_input(
    "Insert cost of replacement (inspections and age-based) ($C_{R}$)", 
    min_value=0.0, value=1.0, 
    help="This parameter represents the cost associated with preventive replacements, whether performed during inspections or when the age-based threshold is reached."
)

cf = st.number_input(
    "Insert cost of failure ($C_{F}$)", 
    min_value=0.0, value=10.0, 
    help="This parameter represents the replacement cost incurred when a component fails."
)

cd = st.number_input(
    "Insert cost of defective by time unit ($C_{D}$)", 
    min_value=0.0, value=0.01, 
    help="This parameter represents the unitary cost associated with the time in which the component stays in defective state for each time unit."
)
